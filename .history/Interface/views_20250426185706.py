...
from .models import DestinoFavorito, Destino
...
def checklist(request):
    if request.method == 'POST':
        ...
        if item_nome:
            ChecklistItem.objects.create(nome=item_nome)
...
def lista_favoritos(request):
    favoritos = DestinoFavorito.objects.filter(usuario=request.user)
    destinos_favoritos = [favorito.destino for favorito in favoritos]
    return render(request, 'favoritos/lista.html', {'destinos_favoritos': destinos_favoritos})


@login_required
def adicionar_favorito(request, destino_id):
    destino = Destino.objects.get(id=destino_id)
    # Verifica se o destino já foi favoritado pelo usuário
    if not DestinoFavorito.objects.filter(usuario=request.user, destino=destino).exists():
        DestinoFavorito.objects.create(usuario=request.user, destino=destino)
    return redirect('lista_favoritos')  # Redireciona para a lista de favoritos

@login_required
def remover_favorito(request, destino_id):
    destino = Destino.objects.get(id=destino_id)
    favorito = DestinoFavorito.objects.filter(usuario=request.user, destino=destino).first()
    if favorito:
        favorito.delete()
    return redirect('lista_favoritos')  # Redireciona para a lista de favoritos
