...
def checklist(request):
    if request.method == 'POST':
        ...
        return redirect('checklist')  # ou o nome correto da URL

    # Pega itens pendentes primeiro, depois os feitos
...
def listar_favoritos(request):
    favoritos = Favorito.objects.filter(user=request.user).values_list('nome_destino', flat=True)
    return JsonResponse(list(favoritos), safe=False)
...
def adicionar_favorito(request):
    if request.method == 'POST':
        ...
        Favorito.objects.get_or_create(user=request.user, nome_destino=nome_destino)
...
def remover_favorito(request):
    if request.method == 'POST':
        ...
        Favorito.objects.filter(user=request.user, nome_destino=nome_destino).delete()
        return JsonResponse({'status': 'ok'})
