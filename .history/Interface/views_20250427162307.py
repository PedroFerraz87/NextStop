...
def roteiro(request):
    if request.method == 'POST':
        ...
        return redirect('home')  # Pode mudar para onde quiser
...
def gerenciar(request):
    ...
    return render(request, 'gerenciar.html', {'viagens': viagens})
...
def editar_viagem(request, viagem_id):
    ...
    if request.method == 'POST':
        destino = request.POST.get('destino')
        data_ida = request.POST.get('data_ida')
        data_volta = request.POST.get('data_volta')

        viagem.destino = destino
        viagem.data_ida = data_ida
        viagem.data_volta = data_volta
        viagem.save()
