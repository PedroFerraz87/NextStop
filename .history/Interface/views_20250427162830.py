...
def gerenciar_viagens(request):
    ...
    for roteiro in roteiros:
        for programacao in roteiro.programacoes.all():
            viagens.append({
                'id': roteiro.id,  # Vamos usar o ID do roteiro pra editar/deletar
    ...
    return render(request, 'gerenciar_viagens.html', context)
...
def editar_roteiro(request, roteiro_id):
    ...
    if request.method == 'POST':
        ...
        if not destino or not data_ida or not data_volta:
            messages.error(request, 'Preencha todos os campos obrigatórios.')
            return redirect(reverse('editar_roteiro', args=[roteiro_id]))
        ...
        # Atualizar programações: primeiro apagar tudo antigo
    ...
    return render(request, 'editar_roteiro.html', context)
...
def excluir_roteiro(request, roteiro_id):
    ...
    # Para segurança: se for GET, pode redirecionar ou mostrar confirmação
    return render(request, 'confirmar_exclusao.html', {'roteiro': roteiro})
