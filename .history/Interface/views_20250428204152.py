...
def excluir_roteiro(request, roteiro_id):
    # Filtra o roteiro para garantir que o usuário está excluindo o próprio roteiro
    roteiro = get_object_or_404(Roteiro, id=roteiro_id, user=request.user)

    if request.method == 'POST':  # Verifica se é um pedido POST
        roteiro.delete()  # Exclui o roteiro
        messages.success(request, 'Roteiro excluído com sucesso!')  # Mensagem de sucesso
        return redirect('gerenciar')  # Redireciona para a página de gerenciamento

    # Se for GET (não é esperado aqui, mas por segurança)
    return redirect('gerenciar')  # Caso o usuário não tenha feito o POST, redireciona para gerenciar
