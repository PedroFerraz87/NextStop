describe('Página de Criar Roteiro', () => {
  beforeEach(() => {
    ...
    cy.visit('/criar-roteiro');  // ajuste para a URL correta
  });

  it('Exibe os campos de formulário e adiciona programações', () => {
    // Verifica se os campos básicos estão visíveis
    cy.contains('Destino');
    ...
    // Verifica se o botão de adicionar programação está funcionando
    cy.get('button').contains('+ Adicionar Programação').click();
    cy.get('input[name="dias[]"]').should('have.length', 2);  // Espera 2 campos de dia
    cy.get('input[name="horarios[]"]').should('have.length', 2); // Espera 2 campos de horário
    cy.get('input[name="locais[]"]').should('have.length', 2); // Espera 2 campos de local

    // Envia o formulário
    cy.get('form').submit();

    // Verifica se a página foi redirecionada ou exibe uma mensagem de sucesso
    cy.url().should('include', '/roteiro'); // Ajuste conforme o redirecionamento esperado
  ...
  it('Exibe mensagem de erro se campos obrigatórios não forem preenchidos', () => {
    cy.get('form').submit();  // Submete sem preencher nada
