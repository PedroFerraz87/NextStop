describe('Teste E2E Alertas e Lembretes - NextStop', () => {
  const login = () => {
    cy.visit('/login');
    cy.get('input[name="username"]').type('usuario_teste');
    cy.get('input[name="password"]').type('senha_teste');
    cy.get('button[type="submit"]').click();
    // Assume redireciona para home após login
    cy.url().should('not.include', '/login');
  };

  beforeEach(() => {
    // Limpar dados e resetar mocks se necessário
    cy.clearCookies();
  ...
  it('Cenário favorável 1 - Notificação correta para evento próximo', () => {
    ...
    // Simular que o usuário tem evento próximo, mockando a API ou preparando DB
    // Exemplo: intercepta a requisição para a view lembretes e retorna dados mockados
    cy.intercept('GET', '/url-da-api-ou-endpoint-lembretes', {
      statusCode: 200,
      body: [
        {
          titulo: 'Evento Próximo',
          evento_iso: new Date(Date.now() + 30 * 60000).toISOString(), // evento em 30 minutos
          min10: 1
        }
      ]
    }).as('getLembretes');

    // Acessa a página de alertas e lembretes
