describe('Funcionalidade: Orçamento de Viagem', () => {
  ...
  beforeEach(() => {
    cy.login('testuser@example.com', 'password123'); // substitua pelo usuário de teste válido
    cy.visit('/orcamento');
  });
  
  
    it('Cenário favorável 1: Deve calcular o valor total estimado da viagem', () => {
      cy.get('input#gasto com hospedagem').type('1000');
      cy.get('input#gasto com passagem').type('1500');
      cy.get('input#gasto com alimentação').type('800');
      cy.get('input#gasto com passeios').type('500');
      cy.get('input#gasto com extras').type('500');
    ...
    it('Cenário desfavorável 1: O valor deve ser 0 ou maior que 0', () => {
      cy.get('input#gasto com hospedagem').type('-1');
      cy.get('#salvar-orcamento').click();
    ...
    it('Cenário favorável 2: Deve permitir ajustar centavos com botão lateral', () => {
      cy.get('input#alimentacao').type('300');
